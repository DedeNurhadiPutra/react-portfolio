...
const Portfolio = () => {
  // for title
  useEffect(() => {
    document.title = "Portfolio | Portfolio Dede Nurhadi Putra";
  }, []);
      ...
      <div className="grid grid-cols-1 gap-4 mt-6 w-5/6 mx-auto md:grid-cols-2 md:w-4/5 md:mt-10 llg:grid-cols-3 llg:w-3/4 llg:mt-6 llg:gap-5 lg:grid-cols-3 lg:w-3/4 lg:mt-6 lg:gap-5">
        {data.map(({ id, image, title, github, demo }) => {
          return (
            <div
              key={id}
              className="text-left bg-[#202020] border-solid border-[1px] border-indigo-500 rounded-2xl py-7 hover:bg-blue-900 ease-in-out duration-500 llg:py-6 llg:w-[98%] llg:h-[22rem] lg:py-6 lg:w-[98%] lg:h-[22rem]"
            >
              <img
                src={image}
                alt={title}
